...
add_action('vc_build_admin_page', 'porto_load_recent_products_shortcode');
add_action('vc_after_init', 'porto_load_recent_products_shortcode');

function porto_shortcode_recent_products($atts, $content = null) {
    ob_start();
    if ($template = porto_shortcode_woo_template('porto_recent_products'))
        include $template;
    return ob_get_clean();
}

function porto_load_recent_products_shortcode() {
    ...
    vc_map(
        array(
            ...
            'description' => __( 'Show recent products', 'porto-shortcodes' ),
            'params' => array(
                array(
                    'type' => 'textfield',
                ...
                array(
                    'type' => 'checkbox',
                    'heading' => __( 'Show Navigation', 'porto-shortcodes' ),
    ...
    if (!class_exists('WPBakeryShortCode_Porto_Recent_Products')) {
        class WPBakeryShortCode_Porto_Recent_Products extends WPBakeryShortCode {
        }
    }
}