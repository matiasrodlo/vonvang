...
add_action('vc_build_admin_page', 'porto_load_top_rated_products_shortcode');
...
function porto_shortcode_top_rated_products($atts, $content = null) {
    ob_start();
    if ($template = porto_shortcode_woo_template('porto_top_rated_products'))
        include $template;
    return ob_get_clean();
}
...
function porto_load_top_rated_products_shortcode() {
    ...
    vc_map(
        array(
            ...
            'description' => __( 'Show top rated products', 'porto-shortcodes' ),
            'params' => array(
                array(
                    'type' => 'textfield',
